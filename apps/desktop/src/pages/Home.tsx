...
export const Home: React.FC = () => {
  ...
  if (!hasAccounts) {
    return (
      <div className="page-home">
        <h1 className="page-title"><Globe size={24} style={{ marginRight: 8, verticalAlign: 'middle' }} />运行状态</h1>
        ...
        <div className="card">
          <h3><Wifi size={18} style={{ marginRight: 8 }} />网络信息</h3>
  ...
  return (
    <div className="page-home">
      ...
      {/* 网络状态卡片 */}
      <div className="card">
        <h3><Wifi size={18} style={{ marginRight: 8 }} />网络状态</h3>
        ...
        <div style={{ display: 'flex', gap: 12 }}>
          {networkStatus === 'disconnected' ? (
            <button className="btn btn-primary" style={{ flex: 1 }} onClick={login}>
              <LogIn size={16} style={{ marginRight: 6 }} />立即连接
            </button>
          ) : networkStatus === 'connected' ? (
            <button className="btn btn-danger" style={{ flex: 1 }} onClick={logout}>
              <LogOut size={16} style={{ marginRight: 6 }} />断开连接
            </button>
          ) : (
            <button className="btn btn-primary" style={{ flex: 1 }} disabled>
              <Loader size={16} style={{ marginRight: 6 }} className="spin" />正在尝试连接...
            </button>
          )}
        </div>
      </div>
      ...
      {/* 当前账号卡片 */}
      <div className="card">
        <h3><User size={18} style={{ marginRight: 8 }} />当前账号</h3>
        {currentAccount ? (
          <div style={{
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 8 }}>
              ...
              <span style={{
                ...
                {currentAccount.isp === 'campus' ? '校园网' :
                 currentAccount.isp === 'cmcc' ? '中国移动' :
                 currentAccount.isp === 'cucc' ? '中国联通' : '中国电信'}
              </span>
            </div>
      ...
      {config?.settings.enableHeartbeat && (
        <div className="card">
          ...
          <div style={{ color: 'var(--text-secondary)', fontSize: '0.9rem' }}>
            ...
            <p style={{ margin: 0 }}>
              状态: <span style={{ color: networkStatus === 'connected' ? '#22c55e' : '#ef4444' }}>
